...
public class CUE4ParseViewModel : ViewModel
{
    ...
    private bool CheckExport(CancellationToken cancellationToken, IPackage pkg, int index, EBulkType bulk = EBulkType.None) // return true once you want to stop searching for exports
    {
        ...
        switch (dummy)
        {
            ...
            case UAnimBlueprintGeneratedClass when isNone && pointer.Object.Value is UAnimBlueprintGeneratedClass animBlueprintGeneratedClass:
            {
                try
                {
                    var animGraph = AnimGraphViewModel.ExtractFromClass(animBlueprintGeneratedClass);
                    if (animGraph.Nodes.Count == 0) return false;

                    var windowTitle = string.IsNullOrEmpty(animGraph.PackageName)
                        ? "Animation Blueprint Graph Viewer"
                        : $"Animation Blueprint Graph Viewer - {animGraph.PackageName}";
                    Application.Current.Dispatcher.Invoke(delegate
                    {
                        Helper.GetWindow<AnimGraphViewer>(windowTitle, () => new AnimGraphViewer(animGraph).Show());
                    });
                    return true;
                }
                catch (Exception e)
                {
                    FLogger.Append(ELog.Warning, () => FLogger.Text($"Failed to extract animation graph: {e.Message}", Constants.WHITE, true));
                    return false;
                }
            }
    ...
    public void ShowMaterialGraph(GameFile entry)
    {
        // temporarily force inlined shader maps on so stripped materials can be rebuilt
        // from their compiled uniform expression bytecode even when the setting is off;
        // exports deserialize lazily, so this must stay set for the whole extraction
        var readShaderMaps = Provider.ReadShaderMaps;
        ...
        try
        {
            ...
            {
                var materialGraph = export switch
                {
                    UMaterialInterface materialInterface => MaterialGraphViewModel.ExtractFromMaterial(materialInterface),
                    UMaterialFunction materialFunction => MaterialGraphViewModel.ExtractFromFunction(materialFunction),
                    _ => null
                };
                if (materialGraph == null || materialGraph.Nodes.Count == 0)
                    continue;

                var windowTitle = string.IsNullOrEmpty(materialGraph.PackageName)
                    ? "Material Graph Viewer"
                    : $"Material Graph Viewer - {materialGraph.PackageName}";
                Application.Current.Dispatcher.Invoke(delegate
                {
                    Helper.GetWindow<MaterialGraphViewer>(windowTitle, () => new MaterialGraphViewer(materialGraph).Show());
                });
                return;
            }

            FLogger.Append(ELog.Warning, () => FLogger.Text($"No material found in '{entry.Name}'", Constants.WHITE, true));
        }
        catch (Exception e)
        {
            FLogger.Append(ELog.Warning, () => FLogger.Text($"Failed to extract material graph: {e.Message}", Constants.WHITE, true));
        }
        finally
        {
