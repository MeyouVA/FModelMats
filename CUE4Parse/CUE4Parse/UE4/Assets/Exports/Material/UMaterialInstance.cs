...
public class UMaterialInstance : UMaterialInterface
{
    ...
    public override void Deserialize(FAssetArchive Ar, long validPos)
    {
        ...
        if (bHasStaticPermutationResource && Ar.Ver >= EUnrealEngineObjectUE4Version.PURGED_FMATERIAL_COMPILE_OUTPUTS)
        {
            ...
            // 4.23/4.24 use the legacy (pre-FMemoryImage) inline shader map format, handled by FMaterialShaderMapLegacy
            if (Ar is { Game: >= EGame.GAME_UE4_23, Owner.Provider.ReadShaderMaps: true })
            {
