        ...
        public FShaderCodeArchive(FArchive Ar)
        {
            ...
            switch (archiveVersion)
            {
                ...
                case 1 when !bIsIoStore:
                    // UE4 pak-cooked library (ShaderCodeLibrary.cpp GShaderCodeArchiveVersion == 1):
                    // TMap<FSHAHash, FShaderCodeEntry> followed by the raw shader code blob;
                    // per-entry offsets are relative to the end of the map
                    SerializedShaders = new FLegacyShaderCodeArchive(Ar);
                    break;
            }
        }
    }
